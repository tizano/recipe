...
export const Image: ImageRelationResolvers = {
  profiles: (_obj, { root }) => {
    return db.image.findUnique({ where: { id: root?.id } }).profiles()
  },
  recipes: (_obj, { root }) => {
    return db.image.findUnique({ where: { id: root?.id } }).recipes()
  },
  ingredients: (_obj, { root }) => {
    return db.image.findUnique({ where: { id: root?.id } }).ingredients()
  },
}
