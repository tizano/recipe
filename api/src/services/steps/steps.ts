...
export const Step: StepRelationResolvers = {
  recipe: (_obj, { root }) => {
    return db.step.findUnique({ where: { id: root?.id } }).recipe()
  },
  subSteps: (_obj, { root }) => {
    return db.step.findUnique({ where: { id: root?.id } }).subSteps()
  },
  parentStep: (_obj, { root }) => {
    return db.step.findUnique({ where: { id: root?.id } }).parentStep()
  },
}
